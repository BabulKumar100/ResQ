...
interface Survivor {
  id: string
  name: string
  status: 'missing' | 'found' | 'rescued' | 'deceased'
  latitude: number
  longitude: number
  qrCode: string
  lastLocation: string
...
export function SurvivorTracker() {
  ...
  const updateDroneTelemetry = () => {
    setDrones(drones.map(drone => {
      if (drone.status === 'active') {
        return {
          ...drone,
          altitude: Math.max(50, Math.min(300, drone.altitude + (Math.random() - 0.5) * 10)),
          ...
          speed: Math.max(0, Math.min(20, drone.speed + (Math.random() - 0.5) * 2)),
          heading: (drone.heading + Math.random() * 5) % 360,
        }
      }
      return drone
    }))
  ...
  const downloadSurvivorReport = (survivor: Survivor) => {
    const report = `
...
Last Updated: ${survivor.foundAt || 'Ongoing search'}
  ...
  const toggleDroneStream = (droneId: string) => {
  ...
  const deployDroneToSurvivor = (droneId: string, survivor: Survivor) => {
    setDrones(drones.map(d => 
      d.id === droneId ? { 
        ...d, 
        status: 'active',
  ...
  const handleAddSurvivor = async () => {
    ...
    const survivor: Survivor = {
      ...
      qrCode: generateQRCode(`sv-${Date.now()}`),
      lastLocation: newSurvivor.lastLocation || 'Unknown',
      contactInfo: newSurvivor.contactInfo,
      injuries: newSurvivor.injuries || 'None reported',
      age: parseInt(newSurvivor.age) || undefined,
      description: newSurvivor.description,
    }
    ...
    try {
      ...
      const mockGoogleData = [
        ...
        },
        {
          ...
          description: 'Missing since 2 hours ago, carrying brown backpack',
        ...
        },
        {
          ...
          description: 'Last seen on surveillance camera',
          injuries: 'Minor'
        },
      ]
      ...
      const newSurvivors = mockGoogleData.map((person) => ({
        ...
        qrCode: generateQRCode(person.name),
  ...
  return (
    <div className="w-full h-full bg-gray-900 text-white p-6 flex gap-4 flex-col lg:flex-row">
      {/* Survivor List */}
      <div className="flex-1 bg-gray-800 rounded-lg p-4 flex flex-col">
        ...
        <div className="flex gap-2 mb-4">
          <button
            ...
            className="flex-1 bg-orange-600 hover:bg-orange-700 px-3 py-2 rounded text-sm flex items-center justify-center gap-2"
          >
          ...
          <button
            ...
            className="flex-1 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-600 px-3 py-2 rounded text-sm flex items-center justify-center gap-2"
          >
        ...
        {showAddForm && (
          <div className="bg-gray-700 p-4 rounded mb-4 space-y-3">
            <input
              ...
              onChange={(e) => setNewSurvivor({ ...newSurvivor, name: e.target.value })}
            ...
            <div className="grid grid-cols-2 gap-2">
              <input
                ...
                onChange={(e) => setNewSurvivor({ ...newSurvivor, age: e.target.value })}
              ...
              <input
                ...
                onChange={(e) => setNewSurvivor({ ...newSurvivor, contactInfo: e.target.value })}
                className="bg-gray-600 px-3 py-2 rounded text-white placeholder-gray-400 text-sm"
              />
            </div>
            <input
              ...
              onChange={(e) => setNewSurvivor({ ...newSurvivor, lastLocation: e.target.value })}
              className="w-full bg-gray-600 px-3 py-2 rounded text-white placeholder-gray-400 text-sm"
            />
            <input
              type="text"
              ...
              onChange={(e) => setNewSurvivor({ ...newSurvivor, injuries: e.target.value })}
              className="w-full bg-gray-600 px-3 py-2 rounded text-white placeholder-gray-400 text-sm"
            />
            <textarea
              placeholder="Description / Additional Info"
              value={newSurvivor.description}
              onChange={(e) => setNewSurvivor({ ...newSurvivor, description: e.target.value })}
        ...
        <div className="flex-1 bg-gray-800 rounded-lg p-4 flex flex-col">
          ...
          <div className="flex gap-2 flex-col mt-auto">
            <button
              ...
              className="w-full bg-orange-600 hover:bg-orange-700 px-4 py-2 rounded flex items-center justify-center gap-2 text-sm"
            >
            ...
            {selectedDrone && (
              <button
                ...
                className="w-full bg-blue-600 hover:bg-blue-700 px-4 py-2 rounded flex items-center justify-center gap-2 text-sm"
              >
    ...
    <div className="w-full lg:w-96 bg-gray-800 rounded-lg p-4 flex flex-col">
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={() => setShowDronePanel(!showDronePanel)} className="text-gray-400 hover:text-white">
          {showDronePanel ? '▼' : '▶'}
        </button>
      </div>
            ...
            {drones.map((drone) => (
              <div
                ...
                className={`p-3 rounded cursor-pointer transition ${
                  selectedDrone?.id === drone.id ? 'bg-orange-600' : 'bg-gray-700 hover:bg-gray-600'
                }`}
              >

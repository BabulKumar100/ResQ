...
interface Resource {
  id: string
  type: 'medical' | 'food' | 'water' | 'shelter' | 'fuel' | 'equipment'
  name: string
  quantity: number
  unit: string
  location: string
  latitude: number
  longitude: number
  status: 'available' | 'depleted' | 'deployed'
...
export function ResourceInventoryMap() {
  const [resources, setResources] = useState<Resource[]>([
    ...
    },
    {
      ...
      longitude: -74.0150,
      status: 'depleted',
      lastRestocked: '2024-01-13T15:00:00Z',
    },
    {
      id: 'res-004',
      type: 'shelter',
      ...
      longitude: -74.0200,
      status: 'available',
      lastRestocked: '2024-01-12T12:00:00Z',
    },
    {
      id: 'res-005',
      type: 'fuel',
  ...
  const updateQuantity = (id: string, delta: number) => {
    setResources(prev => prev.map(r => 
      r.id === id ? { ...r, quantity: Math.max(0, r.quantity + delta) } : r
    ))
  }

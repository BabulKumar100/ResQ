...
export function DroneOverlay() {
  const [drones, setDrones] = useState<DroneStream[]>([
    {
      id: 'drone-001',
      name: 'Alpha-1 Aerial',
      latitude: 40.7128,
  ...
  const [selectedDrone, setSelectedDrone] = useState<DroneStream | null>(drones[0])
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setDrones(prev => prev.map(drone => ({
        ...
        signal: Math.min(100, drone.signal + (Math.random() - 0.5) * 10),
      ...
      <div className="flex-1 bg-black rounded-lg overflow-hidden flex flex-col">
        ...
        <div className="bg-gray-800 px-4 py-3 flex gap-2">
          <button
            ...
            className="flex-1 bg-red-600 hover:bg-red-700 px-3 py-2 rounded text-sm font-bold"
          >

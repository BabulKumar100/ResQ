...
interface ResQMapDashboardProps {
  ...
  return (
    <div className="relative w-full h-screen bg-gray-900 text-white overflow-hidden">
            ...
            {filteredIncidents.length === 0 ? (
              ...
              filteredIncidents.map((incident: any) => (
                <button
                  ...
                  className={`w-full p-3 border-b border-gray-700 text-left hover:bg-gray-700 transition ${
      ...
      {selectedIncident && (
        <div className="absolute right-4 top-24 bottom-4 w-80 bg-gray-800 rounded-lg border border-gray-700 shadow-2xl overflow-auto z-10">
          <div className="p-4">
            ...
            <button className="w-full mt-6 bg-red-600 hover:bg-red-700 px-4 py-2 rounded font-semibold">
              Assign Unit
            </button>
          </div>
        </div>
      ...
      <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 z-20 flex gap-3">
        ...
        <button className="px-4 py-2 rounded font-semibold bg-gray-800 text-gray-400 hover:text-white transition">
          Routing [R]
        </button>
        <button className="px-4 py-2 rounded font-semibold bg-gray-800 text-gray-400 hover:text-white transition">
          Danger Zones [D]
      ...
      <button className="absolute bottom-8 right-8 z-20 w-20 h-20 rounded-full bg-gradient-to-br from-red-500 to-red-700 shadow-2xl hover:shadow-red-500/50 hover:scale-105 transition transform flex items-center justify-center group">
        <AlertTriangle className="w-10 h-10 text-white animate-pulse" />
        <div className="absolute -top-12 right-0 opacity-0 group-hover:opacity-100 transition bg-red-900 text-red-100 px-3 py-2 rounded text-sm whitespace-nowrap">
          SOS [S]
        </div>
      </button>
    </div>

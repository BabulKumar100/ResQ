  ...
  const [predictions, setPredictions] = useState<Prediction[]>([
    {
      ...
      recommendation: 'Position ambulances at exits 15-17',
    },
    {
      id: 'pred-002',
      ...
      severity: 'critical',
      factors: ['High temperature', 'Low humidity', 'Dense construction'],
      recommendation: 'Alert fire departments in area, prep additional resources',
    },
    {
      id: 'pred-003',
      incidentType: 'Flood Risk',
      probability: 62,
      location: 'River District - Low-lying areas',
      latitude: 40.7050,
      longitude: -74.0150,
      timeWindow: 'Next 6 hours',
      severity: 'medium',
      factors: ['Heavy rainfall predicted', 'River levels rising', 'Drainage capacity concerns'],
      recommendation: 'Begin evacuation preparations, deploy pumps',
    },
  ])

  const [selectedPrediction, setSelectedPrediction] = useState<Prediction | null>(predictions[0])
      ...
      {selectedPrediction && (
        <div className="w-96 bg-gray-800 rounded-lg p-4 flex flex-col">
          ...
          <div className="flex gap-2">
            <button className="flex-1 bg-purple-600 hover:bg-purple-700 px-3 py-2 rounded text-sm font-bold">
              Prepare Resources
            </button>
            <button className="flex-1 bg-gray-700 hover:bg-gray-600 px-3 py-2 rounded text-sm">
              Details
            </button>
          </div>
        </div>
